...
| `privateSwapAkdToNight` | Swap AKD for NIGHT, spending the AKD leg as a shielded coin directly instead of a public balance. Both legs real; takes a `recipient: UserAddress` for the tNIGHT payout, same as `swapAkdToNight` |
| `privateSwapNightToAkd` | Swap NIGHT for AKD, minting the AKD leg as a fresh shielded coin instead of crediting a public balance. Both legs real |
...
- All four swap circuits settle tNIGHT for real. The two private ones did not until the security audit: their tNIGHT leg was left out entirely, on the reasoning that `sendUnshielded`/`receiveUnshielded` are transparent and would publicly reveal who traded with this pool and for how much. That reasoning about transparency holds, but the conclusion did not: a circuit whose AKD leg moves real value and whose tNIGHT leg moves none is not a privacy tradeoff, it is `privateSwapAkdToNight` taking a trader's coin and paying nothing back, and `privateSwapNightToAkd` minting real AKD for free. See [hackathon/SECURITY_AUDIT.md](../hackathon/SECURITY_AUDIT.md) findings C-01 and H-01. Both now settle for real, and the cost is an openly transparent tNIGHT leg on the private path. A tNIGHT leg that is both real *and* private would need a genuinely shielded representation of the native token, which this contract cannot create (minting only works for a color the contract itself owns). Still an open research item.
