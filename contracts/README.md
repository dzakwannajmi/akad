...
| `swapAkdToNight` | Swap AKD for NIGHT. Both legs are real: AKD moves from the trader to the pool, and real tNIGHT is paid out of the pool's own custody to the trader's wallet |
...
- `privateSwapAkdToNight`/`privateSwapNightToAkd` are the one place tNIGHT settlement is still simulated, and this is by design rather than an unfinished wiring job: `sendUnshielded`/`receiveUnshielded` are transparent, so using them on a "private" swap would publicly reveal exactly who traded tNIGHT with this pool and how much, defeating the point of the private path. A real *and* private tNIGHT leg would need a genuinely shielded representation of tNIGHT, which this contract cannot create (minting only works for a color the contract itself owns, not the network's native token). Open research item, not solved here.
